...
export class CategoryService {
  ...
  async create(createCategoryDto: CreateCategoryDto) {
    ...
    const payload: Omit<Category, 'updated_at' | 'created_at'> = {
      id: categoryId,
      name: createCategoryDto.name,
    };

    if (createCategoryDto.parent_id) {
      payload.parent_id = UUID.fromString(
    ...
    const institutionCategoryAssignmentModel =
      this._categoryModel.getInstitutionCategoryAssignmentModel();

    if (createCategoryDto.institution_id) {
      await institutionCategoryAssignmentModel.create({
        institution_id: createCategoryDto.institution_id,
  ...
  async findAll(
    filters: CategoryFilters = { is_public: 'false', page: 1, limit: 10 },
  ) {
    ...
    const result = await this._categoryModel.findAndCountAll({
      ...options,
      limit,
      offset,
    });
  ...
  async update(id: string, updateCategoryDto: UpdateCategoryDto) {
    ...
    const category = await this._categoryModel.update(payload, {
      where: { id: categoryId.getValue() },
    });
    if (!category)
  ...
  async remove(id: string) {
    const categoryId = UUID.fromString(id);
    const deletedCategory = await this._categoryModel.delete({
      where: { id: categoryId.getValue() },
    });
    if (!deletedCategory) throw new NotFoundException('Category not found');
    return {
      message: `Category with ID: (${deletedCategory.id}) has deleted successfully!`,
    };
  }
}
