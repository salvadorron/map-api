...
export class FilledFormService {
  ...
  async create(createFilledFormDto: CreateFilledFormDto) {
    ...
    const userIdValue = createFilledFormDto.user_id
      ? UUID.fromString(createFilledFormDto.user_id).getValue()
      : null;
  ...
  async update(id: string, updateFilledFormDto: UpdateFilledFormDto) {
    ...
    if (updateFilledFormDto.records) {
      updateData.records = Object.fromEntries(
        updateFilledFormDto.records.entries(),
      );
    }
    ...
    if (updateFilledFormDto.user_id) {
      updateData.user_id = UUID.fromString(
        updateFilledFormDto.user_id,
      ).getValue();
    }
    ...
    if (updateFilledFormDto.form_version_id) {
      const formVersion = await formVersionModel.findOne({
        where: { id: updateFilledFormDto.form_version_id },
      });
      if (!formVersion) {
        throw new NotFoundException(
          `Form version with ID ${updateFilledFormDto.form_version_id} not found`,
        );
      }
    }
  ...
  async updateToLatestVersion(id: string) {
    ...
    if (!currentFormVersion) {
      throw new NotFoundException(
        `Form version with ID ${currentFilledForm.form_version_id} not found.`,
      );
    }
  ...
  async remove(id: string) {
    ...
    return {
      message: `Filled form with ID: (${filledForm.id}) has deleted successfully!`,
    };
  }
  ...
  async search(q: string, page = 1, limit = 50) {
    ...
    const extRes = await this.db.query(
      "SELECT COUNT(1) AS cnt FROM pg_extension WHERE extname = 'pg_trgm'",
    );
    const hasTrgm = !!(
      extRes &&
      extRes.rows &&
      extRes.rows[0] &&
      Number(extRes.rows[0].cnt) > 0
    );
  ...
  private formatRecordValue(value: any, maxLength: number = 100): string {
    ...
    if (Array.isArray(value)) {
      ...
      const items = value
        .slice(0, 3)
        .map((item) => this.formatRecordValue(item, 30));
    ...
    if (typeof value === 'object') {
      ...
      const items = keys
        .slice(0, 2)
        .map((key) => `${key}: ${this.formatRecordValue(value[key], 20)}`);
  ...
  async generatePDFReport(): Promise<Buffer> {
    ...
    data.forEach((row: any) => {
      const municipalityKey =
        row.municipality_name || row.cod_mun || 'Sin Municipio';
    ...
    return new Promise((resolve, reject) => {
      ...
      Object.keys(organizedData).forEach((municipality, munIndex) => {
        ...
        doc
          .fontSize(16)
          .fillColor('#1a1a1a')
          .text(`Municipio: ${municipality}`, { underline: true });
        doc.moveDown(0.5);

        // Iterar por parroquias
        Object.keys(organizedData[municipality]).forEach(
          (parrish, parIndex) => {
            ...
            doc
              .fontSize(14)
              .fillColor('#333333')
              .text(`Parroquia: ${parrish}`, { indent: 20 });
            doc.moveDown(0.3);
            ...
            forms.forEach((form: any, formIndex: number) => {
              ...
              doc.text(
                new Date(form.filled_form_created_at).toLocaleDateString(
                  'es-ES',
                  {
                    year: 'numeric',
                    month: 'long',
              ...
              if (form.records && typeof form.records === 'object') {
                ...
                if (recordKeys.length > 0) {
                  ...
                  fallbackEntries.forEach((entry) => {
                    ...
                    const formattedValue = this.formatRecordValue(
                      entry.value,
                      120,
                    );
                    ...
                    const valueText =
                      ...
                      entry.value.every((x) => typeof x === 'string')
                        ? entry.value.join(', ')
                        : formattedValue;

                    const lines = String(valueText).match(/.{1,70}/g) || [
                      String(valueText),
                    ];
                    lines.forEach((line) => {
      ...
      doc.text(`Total de formularios: ${data.length}`, { align: 'center' });
