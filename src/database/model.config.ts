...
export class Model<T> {
  ...
  constructor(
    ...
    public primaryKey: keyof T = 'id' as any,
  ) {}
  ...
  async findOne(options: QueryOptions<T> = {}) {
    ...
    const query = `SELECT * FROM ${this.tableName}${clause}${orderClause} LIMIT 1`;
  ...
  async findAll(options: QueryOptions<T> = {}) {
    ...
    if (whereRelation && Object.keys(whereRelation).length > 0) {
      const relationFilteredIds = await this._pgService.runInTransaction<
        string[]
      >(async (client) => {
        ...
        for (const [relationName, relationWhere] of Object.entries(
          ...
          if (!relation) {
            console.warn(
              `Relation "${relationName}" not found for filtering in model ${this.tableName}`,
            );
            continue;
          }
          ...
          if (relation.type === 'belongsToMany') {
            ...
            const { clause: relationClause, values: relationValues } =
              this.buildWhereClause(relationWhere, 1);
            const relatedQuery = `
            ...
            const relatedIds = relatedResult.rows.map(
              (row) => row[relation.otherLocalKey || 'id'],
            );
            ...
            if (relatedIds.length === 0) {
              ...
              const throughQuery = `
                            ...
                            `;
              const throughResult = await client.query<{
                [key: string]: string;
              }>(throughQuery, [relatedIds]);
              relationFilteredIds = throughResult.rows.map(
            ...
            relationFilteredIds = relatedResult.rows.map(
              (row) => row[relation.foreignKey],
            );
            ...
            if (relatedIds.length > 0) {
              ...
              relationFilteredIds = mainResult.rows.map(
                (row) => row[String(this.primaryKey)],
              );
            }
          }
    ...
    const query = `SELECT * FROM ${this.tableName}${clause}${orderClause}${limitClause}`;

    const result = await this._pgService.runInTransaction<T[]>(
      async (client) => {
        const result = await client.query(query, values);
        const rows = result.rows;
  ...
  async create(data: Omit<T, 'updated_at' | 'created_at'>) {
    const keys = Object.keys(data as object) as Array<
      keyof Omit<T, 'updated_at' | 'created_at'>
    >;
  ...
  async update(data: Omit<Partial<T>, 'id'>, options: QueryOptions<T> = {}) {
    ...
    const { clause, values: whereValues } = this.buildWhereClause(
      where,
      dataKeys.length + 1,
    );
  ...
  async findAndCountAll(options: QueryOptions<T> = {}) {
    ...
    const rows = await this._pgService.runInTransaction<T[]>(async (client) => {
      const result = await client.query(
        `SELECT * FROM ${this.tableName}${clause}${orderClause}${limitClause}${offsetClause}`,
        values,
      );
      if (include.length > 0) {
    ...
    const countRes = await this._pgService.runInTransaction<
      { count: string }[]
    >(async (client) => {
    ...
    const count =
      countRes && countRes[0] && typeof countRes[0].count === 'number'
        ? (countRes[0].count as unknown as number)
  ...
  private buildWhereClause(where: object, startingIndex: number = 0) {
    ...
    keys.forEach((key) => {
      ...
      if (value !== null && typeof value === 'object' && 'in' in value) {
        ...
        if (inValues.length === 0) {
          ...
          if (cleanedValues.length === 0) {
            ...
            const allAreUUIDs = cleanedValues.every(
              (v) => typeof v === 'string' && isUUID(v),
            );
            ...
            paramIndex++;
            conditions.push(
              `${String(key)} = ANY($${paramIndex}::${arrayType})`,
            );
      ...
      // Manejo de LIKE / ILIKE
        ...
        value !== null &&
        typeof value === 'object' &&
        ('like' in value || 'ilike' in value)
      ) {
        const likeValue = value.like ?? value.ilike;
        const isILike = value.ilike !== undefined;
        paramIndex++;
   ...
   */
  private buildOrderClause(order?: {
    ...
    const results = await Promise.all(
      rows.map(async (row: any) => {
        ...
        for (const includeItem of include) {
          ...
          if (!relation) {
            console.warn(
              `Relation "${relationName}" not found for model ${this.tableName}`,
            );
            continue;
          }
  ...
  private async loadBelongsTo(
    ...
    if (options?.include && options.include.length > 0) {
      try {
        ...
        console.warn(
          'Nested include failed, falling back to simple query:',
          err,
        );
      }
    }
