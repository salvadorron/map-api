...
class GeoJSONCoordinates implements ValidatorConstraintInterface {
  private _validatePosition = (coord: any[]): boolean =>
    Array.isArray(coord) &&
    coord.length >= 2 &&
  ...
  private _constraints: Record<string, (coordinates: any[]) => boolean> = {
    ...
    LineString: (coordinates) => {
      return (
        coordinates.length >= 2 && coordinates.every(this._validatePosition)
      );
    },
    MultiPoint: (coordinates) => {
      return (
        coordinates.length >= 1 && coordinates.every(this._validatePosition)
      );
    },
    Polygon: (coordinates) => {
